...
def login():
    ...
    try:
        ...
        for doc in users_ref:
            user_data, uid = doc.to_dict(), doc.id
        if user_data:
...
def profile_view(uid):
    try:
        ...
        bmi = round(weight_kg / (height_m * height_m), 1) if height_m > 0 else 0
...
# --- 5. THE AI DASHBOARD (MERGED & CLEANED) ---
...
def dashboard(uid):
    try:
        ...
        # AI Health Analysis (BMI)
        height_m = float(user_doc.get('height', 0)) / 100
        weight_kg = float(user_doc.get('weight', 0))
        bmi = round(weight_kg / (height_m * height_m), 1) if height_m > 0 else 0
        
        bmi_status, color_class = "Not Set", "text-gray-400"
        if bmi > 0:
            if bmi < 18.5: bmi_status, color_class = "Underweight", "text-yellow-400"
            elif 18.5 <= bmi <= 24.9: bmi_status, color_class = "Normal", "text-green-400"
            else: bmi_status, color_class = "Overweight", "text-red-400"
        ...
        return render_template('dashboard.html', 
                             user=user_doc, uid=uid, 
                             bmi=bmi, bmi_status=bmi_status, color_class=color_class, 
                             progress=logs_count, quote=daily_quote)
    except Exception as e:
        return str(e), 500

# --- 6. AI RECOMMENDATION & LOGGING ---
